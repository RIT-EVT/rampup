...
| CPHA | Clock Phase, part of an SPI device's mode setting (paired with CPOL). Controls which clock edge the data is read in on, Rising or Falling.                                                     |
| CPOL | Clock Polarity, part of an SPI device's mode setting (paired with CPHA). It controls whether the idle signal is High or Low.                                                                   |
| CRC | Cyclic Redundancy Check: error-checking bits included at the end of a CAN frame that checks if data got corrupted along the way.                                                               |
| CS | Chip Select: the SPI line that tells one follower device it's being addressed. Every follower has their own, and only one (the follower that is being spoken to) should be active at a time.   |
| DBC | Database CAN: a file format for documenting what a CAN message's bytes mean.                                                                                                                   |
| [FTB](#ftb) | Firmware Test Board                                                                                                                                                                            |
| [GND](#gnd) | Ground                                                                                                                                                                                         |
| [GPIO](#gpio) | General Purpose Input/Output. Normally used in the context of GPIO Pins, which are pins on a uC that the programmer can control to output or receive input data.                               |
...
| [HUDL](#hudl) | Heads Up Display and Lighting. A term used to refer to Phase 4's display node                                                                                                                  |
| [I2C](#i2c) | Inter-Integrated Circuit. A Common protocol used to interface with devices.                                                                                                                    |
...
| NACK | Not Acknowledge, the opposite of ACK.                                                                                                                                                          |
...
| SCL | Serial Clock: I2C's shared clock line (same as SCK but named differently)                                                                                                                      |
| SDA | Serial Data: I2C's shared data line                                                                                                                                                            |
| [SDO](#sdo) | Service Data Object                                                                                                                                                                            |
| [SPI](#spi) | Serial Peripheral Interface. Similarly to I2C, used for communication between (often multiple) devices / chips. Has a higher data transfer rate but requires more pins.                        |
...
| [UART](#uart) | Universal Asynchronous Receiver/Transmitter. Simple communication protocol used to communicate between only two devices. The most common use is UART being used to print messages to a laptop. |
