...
- **Learning Objectives**: what you're meant to walk away understanding
- **Summary**: What you're building and why
- **Task Breakdown**: The steps to follow to complete the Phase
- **Suggestions**: Docs and tips relevant to that phase
- **Completion**: How to know you're done and what to demo to a senior member

Phases are meant to be done in order and build on each other. Each one assumes you're 
comfortable with all the phases before it. There's no deadline on any of this; work 
through it at whatever pace makes sense for how much prior coding/hardware experience 
you have, and lean on senior members early, **they've all gone through this same onboarding and 
found it equally confusing**.
...
If you've never written C++ before, this phase is designed to be simple enough to allow you to not drown immediately; 
but you still are going to be learning the language and the hardware workflow at
the same time. It's expected that this can be confusing; the actual sum-of-two-numbers logic is simple
on purpose so the rest of your effort can go toward getting comfortable with
syntax, the build/flash loop, and reading a serial console.
...
### Suggestions
- Make sure you've completed [`docs/GETTING_STARTED.md`](docs/GETTING_STARTED.md)
first - you should be able to build, flash, and see the `blink` sample working
before you write any rampup code.
- New to C++? [learncpp.com](https://www.learncpp.com/) is a solid free
reference for syntax questions as you go. Please don't read it front to
back, just search it when you hit something confusing or unfamiliar (or ask an AI to explain).
...
[here](https://sites.google.com/g.rit.edu/evt-home-page/firmware-team/getting-started/running-code))
    ...
    - `screen` or `cereal` for MacOS/Linux
...
### Completion
When you believe you've gotten everything working, try holding your thumb against
...
communicate between the uC and a 3-axis accelerometer, a special IC capable
of sensing the acceleration of the chip. For our purposes, we're simply going
to read the acceleration of all three axes, convert it to G-force, and print
it out. You'll still need the Saleae and FTB, but you'll need to switch out
the TMP117 board for the ADXL345 board. For this phase, the header file
hasn't been provided for you, so you'll have to write that yourself. The
files you need to edit are `include/dev/ADXL345.hpp`, `src/dev/ADXL345.cpp`,
and `targets/phase3/main.cpp`.

> IMPORTANT: The Chip Select (CS) pin on the FTB's click board slot is not the default SPI_CS pin, it is instead PA_9!
...
4. Implement the functions defined in the header file and initialize the
ADXL345 sensor.
5. Write the target code to call the functions and print all accelerations in
g-force over UART.
...
### Completion
Once you think it's working, you should see a g-force on the z-axis around 1
due to gravity. Try standing the board up on one end to see if the
acceleration shifts to one of the other axes, then try it again on the
perpendicular axis. You should be able to see a positive and negative
acceleration due to gravity on each axis. When you're happy with its
performance, have a senior member look at your Saleae output and your code.
If it looks good, you can start on the last phase.
...
team would make that reports temperature and acceleration data over CAN. We
haven't made a PCB that does that, but you can prototype one using the FTB. For
this phase, you'll need a click board for the TMP117 or the ADXL345 and an
...
This is the hardest phase, mostly because of CANopen, the standard we use on
top of CAN. It's significantly more involved than UART, I2C, or SPI, so before you
start editing `RampupBoard.hpp`, read [`docs/CANOPEN_GUIDE.md`](docs/CANOPEN_GUIDE.md),
which walks through what an object dictionary, SDO, and PDO actually are, and explains 
what the existing `RampupBoard.hpp` code is already doing so you're not redoing already 
completed work or attempting to reverse-engineer it from scratch.
...
### Suggestions
- Read [`docs/CANOPEN_GUIDE.md`](docs/CANOPEN_GUIDE.md) first. It's written
specifically for the `RampupBoard.hpp` code you're about to edit.
...
- Look at the canopen_sample and canopen_tpdo samples in EVT-core for examples,
and at `targets/hudl/main.cpp` in this repo for a fully working CANopen main
loop you can pattern yours after.
